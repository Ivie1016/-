...
namespace LiyMIS01
{
    public partial class frmTclass : Form
    {
        ...
        private void LoadTeacherComboBox()
        {
            try
            {
                ...
                sql = "SELECT ly_tno01, ly_tname01 FROM liye.Liy_Teachers01 ORDER BY ly_tno01";
                ds = con.Getds(sql);

                comboBox1.Items.Add("所有教师");
                foreach (DataRow row in ds.Tables[0].Rows)
                {
                    comboBox1.Items.Add($"{row["ly_tno01"]}-{row["ly_tname01"]}");
                }
                comboBox1.SelectedIndex = 0;
            }
            catch (Exception ex)
            {
                MessageBox.Show("加载教师数据失败！\n" + ex.Message,
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (comboBox1.SelectedItem.ToString() == "所有教师")
                {
                    MessageBox.Show("请选择具体教师", "提示",
                        MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }

                string teacherNo = comboBox1.SelectedItem.ToString().Split('-')[0];
                string semesterCondition = comboBox2.SelectedItem.ToString() != "所有学期" ?
                    $" AND cl.ly_semester01 = '{comboBox2.SelectedItem}'" : "";

                sql = $@"SELECT c.ly_cname01 AS 课程, cl.ly_ctime01 AS 时间, 
                         cl.ly_cplace01 AS 地点, cl.ly_class01 AS 班级
                         FROM liye.Liy_Class01 cl
                         JOIN liye.Liy_Courses01 c ON cl.ly_cno01 = c.ly_cno01
                         WHERE cl.ly_tno01 = '{teacherNo}' {semesterCondition}
                ...
                foreach (DataRow row in ds.Tables[0].Rows)
                {
                    string timeStr = row["时间"].ToString();
                    string courseInfo = $"{row["课程"]}\n{row["班级"]}\n{row["地点"]}";
                ...
                MessageBox.Show($"已加载 {ds.Tables[0].Rows.Count} 条课程记录",
